-- Backfill group tables in case migration history was repaired without physical DDL application.
...
alter table public.group_chats add column if not exists announcement_mode boolean not null default false;
alter table public.group_chats add column if not exists invite_requires_approval boolean not null default false;
alter table public.group_chats add column if not exists restrict_forwarding boolean not null default false;
alter table public.group_chats add column if not exists key_version int not null default 1;
alter table public.group_chats add column if not exists updated_at timestamptz not null default now();

alter table public.group_chat_members add column if not exists role text not null default 'member';
alter table public.group_chat_members add column if not exists muted_until timestamptz;
alter table public.group_chat_members add column if not exists joined_at timestamptz not null default now();
alter table public.group_chat_members add column if not exists last_read_at timestamptz;
alter table public.group_chat_members add column if not exists last_delivered_at timestamptz;
